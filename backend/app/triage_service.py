...
ProgressCb = Callable[[str, str, int, dict[str, Any] | None], Awaitable[None]]
...
async def build_live_result(
    *,
    job_id: str = "live-snapshot",
    ...
    cache = DiskCache(cache_root, ttl_seconds=0 if force_refresh else 12 * 3600)

    async def progress_checked(
        stage: str, message: str, pct: int, detail: dict[str, Any] | None = None
    ) -> None:
        if check_cancelled and check_cancelled() and stage not in {"cancelled", "error", "done"}:
    ...
    async with httpx.AsyncClient() as client:
        ...
        async def nvd_cb(stage: str, detail: dict[str, Any]) -> None:
            ...
            if status == "start":
                await progress_checked("nvd", f"Querying NVD for {label}…", 20, detail)
            else:
                await progress_checked(
                    "nvd",
        ...
        await progress_checked("epss", "Scoring exploit likelihood via FIRST EPSS…", 65)
        epss_attached = 0
        try:
            epss = await fetch_epss_for_cves(client, cache, [f.cve_id for f in findings])
            for f in findings:
            ...
            await progress_checked("epss", f"EPSS attached for {epss_attached} CVEs.", 75)
        ...
        try:
            ...
            await progress_checked("ibm", f"IBM enrichment degraded ({ibm_exc})", 88)

        await progress_checked(
            "rank",
            "Applying counter-lever ranking (CISA · EPSS · OWASP · CVSS · PSIRT)…",
            92,
        )
...
async def run_triage(job_id: str, force_refresh: bool = False) -> None:
    ...
    try:
        result = await build_live_result(
            ...
            check_cancelled=lambda: store.is_cancelled(job_id),
        )
